...
% sheet pile using the conformal mapping technique
...
xf = 2.75;          % Width factor: controls the x-axis limit of the plot
%=========================================================================
% DERIVED PARAMETERS::
A = [-2*b1 0];B = [-b1 0];C = [0 0];D = [0 -s];F = [b1 0];
h = h1-h2;
%=========================================================================
...
for ii = 1:fl
    leg4 = plot(real(zx(ii,:)),imag(zx(ii,:)),'k--','linewidth',1.5);
end
for ii = 1:fl
    leg5 = plot(real(zy(ii,:)),imag(zy(ii,:)),'r','linewidth',1.5);
end
xlabel('Width (m)','fontsize',16); 
ylabel('Height (m)','fontsize',16)
legend([leg1 leg2 leg3 leg4 leg5],{'Dam outline','Sheet pile',...
    'Water level','Flow lines','Equipotential lines'},...
...
for ii = 1:2:fl
    ...
    elseif mod((ii+1)/2,2) == 0
        xc2 = max(real(zx(ii,:)));
        h = text(xc2,0.5,t2,'HorizontalAlignment','left','Color','k',...
            'FontSize',16); 
        set(h,'Rotation',90);
